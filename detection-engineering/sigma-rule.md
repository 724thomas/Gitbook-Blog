...
**feedback: 방식이 버전에 따라 다름**

**현재: YYYY-MM-DD.** 과거: YYYY/MM/DD

* 비표준화와 파싱오류: 과거 형식으로는 SIEM에서 읽기가 어렵고 파싱 오류가 자주 발생했습니다.
* 정렬 및 검색 효율성: ISO 8601(- 구분자)를 사용하면 시간 순서가 보장됩니다.
* 마이그레이션: SigmaHQ가 운영하는 Github Repo에 있는 3\_000개 이상의 규칙은 수정이 됬지만, Private한 규칙들은 따로 수정을 해야합니다.

```bash
pip install sigma-cli
sigma check /path/to/rules/
sigma check my_awesome_rule.yml
```



#### 메타데이터의 require와 optional 여부를 인식하고 있어야 함

```yaml
(필수)title: Shadow Copy Deletion via WMIC
(필수)id: f24830b4-bc10-46db-817a-24df74643033
related:
    - id: b12a7a44-bc10-46db-817a-24df74643033 (필수 - related 사용 시)
      type: derived (필수 - related 사용 시)
status: stable (옵션 - 권장)
description: Detects the deletion of volume shadow copies using WMIC.exe, often used by ransomware to prevent recovery.
author: Sigma Project Team
date: 2026-04-16
modified: 2026-04-17
references:
...
tags:
    ...
    - attack.impact
(필수)logsource:
    category: process_creation (옵션 - 권장)
    product: windows
    service: sysmon
    definition: 'Requires Sysmon Event ID 1 (Process Creation) logging'
(필수)detection: 
    (필수)selection: 
    ...
    (필수)condition: selection and not filter
    timeframe: 5m (옵션 - Aggregation 사용 시 필수)
fields:
    - CommandLine
    - ParentCommandLine
    - User
...
logsource: (필수)
    ...
    product: windows (옵션)
    service: sysmon (옵션)
    definition: 'Requires Microsoft-Windows-Sysmon/Operational or Security Event ID 4688' (옵션)
```

{% embed url="https://github.com/SigmaHQ/sigma/blob/master/tests/logsource.json" %}



#### Sigma tags (Ver 2.1.0, 2025-08-02)

Namespaces

* attack: MITRE ATT\&CK 카테고리 (eg. attack.t1053.005 / attack.lateral\_movement)
* car: MITRE Cyber Analytics Repository 링크 (eg. car.2016-04-005)
* cve: MITRE CVE 카테고리 (eg. cve.2021-44228)
* d3fend: MITRE D3FEND 카테고리 (eg. d3fend.d3f-pna)
* detection: SigmqHQ에서 관리하는 목적별 규칙 분류 (eg. detection.threat-hunting)
* stp: MITRE Summiting the Pyramid(고통의 피라미드)에 따른 레이팅
* tlp: Traffig Light Protocol (정보공유등급)
