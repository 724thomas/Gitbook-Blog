...
* Collection 인터페이스\
  ...
  공통적으로Add, Remove, Size, Iterator 메서드가 있습니다.
* List 인터페이스\
...
(Vector는 동기화된 자료구조이고, 상속받는 Stack도 동기화된 자료구조.)\
코테에서는 Stack보다는 ArrayDeque가 성능이 더 좋습니다.
...
* HashSet - 빠른 조회, 성능, 정렬X
* TreeSet - 정렬 (우선순위큐와의 차이는 중복허용과, 모든 요소 정렬 / 하나의 원소만 정렬)
* LinkedHashSet - 삽입 순서를 정한다.
