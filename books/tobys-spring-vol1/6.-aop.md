...
스프링 AOP의 목적은 트랜잭션과 같은 부가기능을 핵심 비즈니스 로직으로부터 완전히 분리하고, 적용 대상을 선언적으로 지정하여 컨테이너가 자동으로 프록시를 생성하도록 만드는 데 있습니다. 이 절에서는 ProxyFactoryBean 기반의 수동 프록시 설정을 제거하고, 자동 프록시 생성기와 포인트컷 고도화, 그리고 표현식 기반 포인트컷으로 발전시키는 과정을 정리합니다. 결과적으로 서비스 계층 코드는 트랜잭션을 알지 못하며, 트랜잭션 적용 여부는 포인트컷 규칙에 의해 결정됩니다.

***

### 6.5.1 자동 프록시 생성

**ProxyFactoryBean 방식의 구조적 한계**

ProxyFactoryBean을 도입하면 프록시 구현 클래스 작성은 사라집니다. 하지만 다음과 같은 문제가 남습니다.

* 타깃 빈이 추가될 때마다 ProxyFactoryBean 빈을 새로 등록해야 합니다.
* 대부분의 설정은 동일하고 target 프로퍼티만 달라집니다.
* 설정 복사 과정에서 실수가 발생할 가능성이 있습니다.
* 설정 파일이 점점 비대해집니다.

즉, 프록시 구현 중복은 제거되었지만 **프록시 설정 중복**은 여전히 남아 있습니다.

***

**DefaultAdvisorAutoProxyCreator의 도입**

해결 방법은 프록시 생성을 컨테이너에 위임하는 것입니다.

`DefaultAdvisorAutoProxyCreator`는 빈 후처리기(BeanPostProcessor)의 일종으로, 빈 생성 과정에 개입하여 자동으로 프록시를 생성합니다.

```
<bean class="org.springframework.aop.framework.autoproxy.DefaultAdvisorAutoProxyCreator"/>
```

이 설정이 등록되면 컨테이너는 다음 순서로 동작합니다.

1. 빈을 생성합니다.
2. 등록된 Advisor들을 조회합니다.
3. 각 Advisor의 포인트컷으로 해당 빈이 대상인지 판단합니다.
4. 대상이라면 프록시를 생성합니다.
5. 원본 대신 프록시 객체를 최종 빈으로 등록합니다.

이제 개발자는 ProxyFactoryBean을 더 이상 만들 필요가 없습니다.

***

**자동 프록시 환경에서 포인트컷의 확장**

이전에는 특정 타깃을 명시했기 때문에 메서드만 선별하면 되었습니다.\
자동 프록시 생성 환경에서는 모든 빈이 후보가 됩니다.

따라서 포인트컷은 두 단계를 모두 판단해야 합니다.
...
* ClassFilter: 적용할 클래스인지 판단
* MethodMatcher: 적용할 메서드인지 판단

두 조건이 모두 만족되어야 어드바이스가 적용됩니다.

***

**클래스 필터를 포함한 포인트컷 구현**

클래스 이름과 메서드 이름을 동시에 조건으로 사용하는 포인트컷 구현 예시는 다음과 같습니다.

```
class NameMatchClassMethodPointcut : NameMatchMethodPointcut() {

    fun setMappedClassName(mappedClassName: String) {
        setClassFilter(SimpleClassFilter(mappedClassName))
    }

    private class SimpleClassFilter(
...
```
<bean id="transactionPointcut"
      class="...NameMatchClassMethodPointcut">
...
* 클래스 이름이 ServiceImpl로 끝나는 클래스
* 메서드 이름이 upgrade로 시작하는 메서드

이제 해당 조건을 만족하는 모든 빈에 자동으로 트랜잭션이 적용됩니다.

***

### 6.5.2 DefaultAdvisorAutoProxyCreator의 적용

자동 프록시 생성기를 실제 서비스 계층에 적용하면 구조가 단순해집니다.
...
트랜잭션 적용 여부는 포인트컷 규칙에 의해 자동 결정됩니다.

***

**테스트용 구현 클래스의 주의점**

포인트컷이 클래스 이름 패턴을 기준으로 동작하기 때문에, 테스트용 구현 클래스 역시 조건을 만족해야 합니다.

예를 들어 다음과 같이 예외를 발생시키는 테스트 구현을 작성합니다.

```
class TestUserServiceImpl : UserServiceImpl() {

    private val failId = "madnite1"

    override fun upgradeLevel(user: User) {
        if (user.id == failId) {
            throw RuntimeException("강제 예외 발생")
        }
...
포인트컷이 `*ServiceImpl` 조건을 사용한다면, 클래스 이름도 반드시 ServiceImpl로 끝나야 합니다. 그렇지 않으면 트랜잭션이 적용되지 않습니다.

***

**설정 상속을 통한 중복 제거**

테스트용 빈은 parent 속성을 이용해 기존 설정을 상속할 수 있습니다.
...
이렇게 하면 userDao, mailSender 등의 의존성 설정을 다시 작성하지 않아도 됩니다.

***

**자동 프록시 적용 여부 확인**

자동화된 구조에서는 적용 여부를 명확히 검증해야 합니다.

```
@Test
fun proxyCheck() {
    assertTrue(Proxy.isProxyClass(userService.javaClass))
}
```

이 테스트는 userService가 실제 구현체가 아니라 프록시 객체임을 확인합니다.

***

### 6.5.3 포인트컷 표현식을 이용한 포인트컷

이름 기반 포인트컷은 한계가 있습니다.

* 반환 타입 조건 지정이 어렵습니다.
* 파라미터 조건 지정이 복잡합니다.
* 예외 타입 조건 지정이 어렵습니다.
* 패키지 단위 조건 지정이 제한적입니다.

이를 해결하기 위해 AspectJ 표현식을 사용합니다.
...
```
execution([접근제어자] 반환타입 패키지.클래스.메서드(파라미터) [throws 예외])
```
...
* `*` : 모든 값
* `..` : 패키지 하위 전체 또는 파라미터 임의 개수
* `||` : OR 조건

표현식 기반 포인트컷은 클래스 필터와 메서드 매처를 직접 구현할 필요를 제거합니다.

***

### 6.5.4 AOP의 핵심 개념 정리

* **타깃(Target)**: 부가기능이 적용될 객체
* **어드바이스(Advice)**: 실제 부가기능 로직
* **조인 포인트(Join Point)**: 어드바이스가 적용 가능한 지점
* **포인트컷(Pointcut)**: 조인 포인트 중 실제 적용 대상 선별
* **어드바이저(Advisor)**: 포인트컷 + 어드바이스
* **프록시(Proxy)**: 타깃을 감싸 부가기능을 적용하는 객체
* **애스펙트(Aspect)**: 어드바이스와 포인트컷의 모듈 단위
...
AspectJ는 바이트코드를 수정하는 방식이며, 필드 접근이나 생성자 호출까지 확장 가능합니다.

일반적인 트랜잭션 적용에는 스프링 AOP로 충분합니다.

***

#### 6.5.6 AOP 네임스페이스

XML 설정을 간결하게 만들기 위해 AOP 네임스페이스를 사용할 수 있습니다.
...
트랜잭션은 단순히 commit/rollback으로 끝나는 기술이 아닙니다.\
스프링에서는 트랜잭션의 동작을 속성으로 정의하며, 이 속성 조합에 따라 트랜잭션의 시작, 참여, 중단, 격리, 롤백 여부가 결정됩니다.\
`TransactionDefinition`은 트랜잭션 속성을 정의하는 인터페이스이며, `DefaultTransactionDefinition`은 그 기본 구현체입니다.\
`TransactionInterceptor`는 메서드 실행 전후에 트랜잭션 경계를 설정하며, 이때 정의된 트랜잭션 속성을 기반으로 동작합니다.
...
트랜잭션 매니저는 `getTransaction(TransactionDefinition)`을 통해 트랜잭션을 처리합니다.\
이 메서드는 항상 새로운 트랜잭션을 시작하는 것이 아니라, 현재 실행 중인 트랜잭션 존재 여부와 전파 속성을 기준으로 다음 중 하나를 선택합니다.
...
전파 속성은 “이미 트랜잭션이 있는 상황에서 어떻게 동작할 것인가”를 정의합니다.

* `PROPAGATION_REQUIRED`\
  진행 중인 트랜잭션이 없으면 새로 시작하고, 있으면 참여합니다. 기본값입니다.
* `PROPAGATION_REQUIRES_NEW`\
  항상 새로운 트랜잭션을 시작합니다. 기존 트랜잭션은 잠시 보류됩니다.
* `PROPAGATION_NOT_SUPPORTED`\
  트랜잭션 없이 실행합니다. 이미 진행 중이라면 일시 중단합니다.

전파 속성은 서비스 계층 설계와 밀접하게 연결됩니다.\
하나의 서비스 메서드가 여러 DAO 작업을 묶는다면 `REQUIRED`가 자연스럽고,\
독립적인 로그 기록이나 이력 저장은 `REQUIRES_NEW`가 적절할 수 있습니다.

***

_**격리수준**_

격리수준은 동시에 실행되는 트랜잭션 간의 데이터 간섭을 제어합니다.
...
격리수준이 높을수록 데이터 정합성은 강해지지만, 성능 비용이 증가합니다.\
일반적으로는 DB 기본값을 유지하고, 특별한 경우에만 조정합니다.

***

_**제한시간**_

`timeout`은 트랜잭션의 최대 실행 시간을 의미합니다.\
지정 시간이 초과되면 예외가 발생하고 롤백됩니다.\
대기 시간이 길어질 수 있는 배치 작업 등에 유용합니다.

***

_**읽기전용**_

`readOnly = true`로 설정하면 해당 트랜잭션은 조회 목적임을 명시합니다.

* DB 드라이버 수준에서 최적화가 가능할 수 있습니다.
* 트랜잭션 내부에서 데이터 변경 시 예외가 발생할 수 있습니다.

조회 메서드(`get*`, `find*`)에 적용하는 것이 일반적인 전략입니다.
...
필요한 경우 `rollbackFor`, `noRollbackFor` 등을 통해 조정할 수 있습니다.

***

### 6.6.2 트랜잭션 인터셉터와 메서드 패턴

`TransactionInterceptor`는 메서드 이름 패턴에 따라 서로 다른 트랜잭션 속성을 적용할 수 있습니다.\
이를 통해 코드 수정 없이 설정만으로 트랜잭션 전략을 변경할 수 있습니다.

예시 전략은 다음과 같습니다.

* `get*` → `readOnly`
* `upgrade*` → `REQUIRES_NEW`
* `*` → `REQUIRED`

속성 문자열은 다음 구조를 가집니다.

```kotlin
PROPAGATION_NAME, ISOLATION_NAME, readOnly, timeout_숫자
```

***

#### 6.6.3 포인트컷과 트랜잭션 적용 전략

트랜잭션은 보통 서비스 계층 전체에 적용합니다.

포인트컷은 “어떤 메서드를 트랜잭션 대상으로 삼을 것인가”를 결정하고,\
트랜잭션 속성은 “그 메서드에 어떤 정책을 적용할 것인가”를 결정합니다.

권장 전략은 다음과 같습니다.

1. 서비스 계층 전체를 포인트컷으로 지정합니다.
2. 대부분은 `REQUIRED` 기본값을 사용합니다.
3. 조회 계열 메서드만 `readOnly`로 분리합니다.
4. 예외적인 메서드만 별도 속성을 지정합니다.

***

#### 프록시 기반 AOP의 주의점

프록시 기반 AOP는 클라이언트 → 프록시 → 타깃 호출 경로에서만 동작합니다.

같은 클래스 내부에서 다른 메서드를 호출하면 프록시를 거치지 않습니다.\
따라서 내부 호출에는 트랜잭션 속성이 적용되지 않을 수 있습니다.

이 경우 해결 방법은 다음과 같습니다.

* 구조를 분리하여 외부 호출이 되도록 설계합니다.
* AspectJ 같은 바이트코드 기반 AOP를 사용합니다.

***

#### 트랜잭션 속성 테스트

읽기전용 트랜잭션이 제대로 동작하는지 테스트할 수 있습니다.

조회 메서드 안에서 의도적으로 쓰기 작업을 수행하면 예외가 발생해야 합니다.
...
class TestUserService(
    ...
    fun getAllWithWriteAttempt(): List<User> {
        val users = userService.getAll()
        users.forEach {
            userService.update(it) // readOnly 트랜잭션이면 예외 발생
        }
        return users
    }
}
...
class TransactionTest(
    private val testService: TestUserService
) {

    @Test
    fun readOnlyShouldFailOnWrite() {
        assertThrows(TransientDataAccessResourceException::class.java) {
            testService.getAllWithWriteAttempt()
        }
    }
}
```
...
포인트컷 표현식과 메서드 이름 패턴을 이용해 트랜잭션을 일괄 적용하는 방식은 대부분의 상황에서 충분합니다.\
그러나 클래스나 메서드 단위로 서로 다른 세밀한 트랜잭션 속성이 필요해지면 설정이 복잡해집니다.\
이때 스프링은 **트랜잭션 속성을 애노테이션으로 직접 선언하는 방식**을 제공합니다.\
즉, 포인트컷이 트랜잭션 대상을 찾는 것이 아니라, 트랜잭션이 필요한 위치에 속성 정보를 명시하고 이를 기반으로 어드바이스가 동작합니다.

***

### 6.7.1 트랜잭션 애노테이션

스프링에서 제공하는 트랜잭션 애노테이션은 `@Transactional`입니다.\
이 애노테이션은 메서드, 클래스, 인터페이스에 적용할 수 있습니다.

`@Transactional`이 제공하는 주요 속성은 다음과 같습니다.
...
애노테이션 기반 트랜잭션은 내부적으로 `TransactionInterceptor`를 사용합니다.\
차이점은 트랜잭션 속성을 메서드 패턴이 아니라 `AnnotationTransactionAttributeSource`를 통해 읽어온다는 점입니다.

동작 흐름은 다음과 같습니다.

1. 포인트컷이 `@Transactional`이 붙은 대상 검색
2. `AnnotationTransactionAttributeSource`가 애노테이션 속성 분석
3. `TransactionInterceptor`가 해당 속성으로 트랜잭션 적용

따라서 애노테이션이 곧 트랜잭션 속성 정의이자 포인트컷 기준이 됩니다.

***

#### 대체 정책(Fallback Rule)

스프링은 `@Transactional`을 적용할 때 다음 순서로 속성을 탐색합니다.
...
즉, 메서드에 직접 선언된 애노테이션이 가장 우선합니다.\
클래스 레벨에 선언되어 있으면 해당 클래스의 모든 메서드에 공통 적용됩니다.\
인터페이스에 선언한 경우 프록시 방식에 따라 동작이 달라질 수 있으므로 주의가 필요합니다.

***

#### 트랜잭션 애노테이션 사용 설정

애노테이션 기반 트랜잭션을 사용하려면 다음 설정이 필요합니다.

```kotlin
<tx:annotation-driven />
```

이 설정 하나로 다음이 자동 등록됩니다.
...
1. Service로 끝나는 클래스의 모든 메서드가 후보
2. 그중에서
   * 이름이 `upgrade`로 시작 → REQUIRED
   * 이름이 `add`로 시작 → REQUIRED
   * 이름이 `get`으로 시작 → readOnly

이 방식의 특징은 메서드 이름이 트랜잭션 정책을 결정합니다.



현재는 메서드가 3개 이기때문에 괜찮지만, 예를 들어

* upgradeLevels → REQUIRED
* specialUpgrade → REQUIRES\_NEW
* batchUpgrade → REQUIRED + timeout 60초
* getUser → readOnly
* getUserForUpdate → REQUIRED
* addUser → REQUIRED
* addUserWithoutTx → 트랜잭션 없음

이렇게 요구사항이 추가된다면,

```xml
<prop key="upgrade*">PROPAGATION_REQUIRED</prop>
<prop key="specialUpgrade">PROPAGATION_REQUIRES_NEW</prop>
<prop key="batchUpgrade">PROPAGATION_REQUIRED,timeout_60</prop>
<prop key="get*">PROPAGATION_REQUIRED,readOnly</prop>
<prop key="getUserForUpdate">PROPAGATION_REQUIRED</prop>     //
<prop key="add*">PROPAGATION_REQUIRED</prop>
```
...
getUserForUpdate는 \<prop key="get\*">PROPAGATION\_REQUIRED,readOnly\</prop>에 걸리게 됩니다.\
근데 readOnly가 되면 안되서, 신경써야하는 것들이 많아집니다.

#### 문제2

메서드 이름을 바꾸는 순간 정책이 깨지게 됩니다

#### 문제3

각 메서드는 XML을 보지 않는 이상 정책을 보기가 불편해집니다.

***

### 6.7.2 트랜잭션 애노테이션 적용

서비스 인터페이스에 기본 트랜잭션을 선언하고,\
조회 메서드에만 읽기전용 속성을 추가하는 방식이 일반적인 전략입니다.
...
타깃 클래스에만 `@Transactional`을 둘 경우,\
인터페이스에 선언된 설정은 무시될 수 있습니다.
...
이 경우 클래스 레벨 설정이 우선 적용됩니다.\
인터페이스에 설정한 `readOnly` 속성이 기대대로 동작하지 않을 수 있습니다.

스프링 트랜잭션은, 이 호출을 어떤 속성으로 트랜잭션을 실행할 것인가 결정하고,\
이 결정을 위해 스프링은 후보들을 아래 순서대로 찾아서 트랜잭션 속성을 찾습니다.

* 호출된 **메서드**에 `@Transactional`이 있는가
* 그 메서드가 속한 **클래스**에 `@Transactional`이 있는가
* (프록시 방식에 따라) **인터페이스 메서드/인터페이스 타입**에 `@Transactional`이 있는가 (JDK proxy)
* (프록시 방식에 따라) **구현 클래스 메서드/구현 클래스 타입**에 `@Transactional`이 있는가 (CGLIB)



**인터페이스에는 readOnly, 구현 클래스에는 클래스 레벨 기본값**
...
@Transactional // readOnly 지정 없음 (기본은 readOnly=false 성격)
class UserServiceImpl : UserService {
...
* `getUser()`는 인터페이스에 `readOnly = true`가 있으니 읽기 전용이어야 한다.

실제에서 문제가 생길 수 있는 경우:

* 프록시/메서드 매칭 과정에서 “이 호출의 트랜잭션 속성”을 뽑을 때 **구현 클래스의 클래스 레벨 `@Transactional`** 을 먼저 적용하거나,
* 인터페이스의 `readOnly`를 “더 구체적인 설정”으로 인정하지 못하고
* 결과적으로 `readOnly=true`가 누락되거나 덮이는 상황이 발생합니다.



따라서 프록시 방식(AOP)을 사용하는 경우에는\
일반적으로 **인터페이스에 선언하는 것이 안전합니다.**
...
* 설정이 직관적입니다.
* 포인트컷 복잡도가 줄어듭니다.
* IDE 자동완성 및 컴파일 단계 검증이 가능합니다.
* 메서드 단위로 세밀한 제어가 가능합니다.



## 6.8 트랜잭션 지원 테스트

트랜잭션 전파와 동기화 메커니즘 덕분에 스프링은 테스트 코드에서도 트랜잭션을 제어할 수 있습니다.\
선언적 트랜잭션이 서비스 계층에 적용되어 있더라도, 테스트 코드에서 트랜잭션 매니저를 사용하면 여러 서비스 메서드를 하나의 트랜잭션으로 묶거나 롤백 여부를 검증할 수 있습니다.\
또한 스프링 테스트 프레임워크는 테스트에 `@Transactional`을 적용하여 자동 롤백 테스트를 지원합니다.

***

### 6.8.1 선언적 트랜잭션과 트랜잭션 전파 속성

전파 속성 `REQUIRED`는 이미 시작된 트랜잭션이 있으면 참여하고, 없으면 새로 시작합니다.

예를 들어 `UserService.add()`가 `REQUIRED`라면 다음과 같이 동작합니다.

* 독립적으로 호출되면 새로운 트랜잭션이 시작됩니다.
* 이미 다른 메서드에서 트랜잭션이 시작된 상태라면 해당 트랜잭션에 참여합니다.

이 특성 덕분에 여러 메서드를 하나의 비즈니스 트랜잭션으로 묶을 수 있습니다.

`processDailyEventRegistration()`과 같은 메서드가 트랜잭션 경계라면, 내부에서 호출되는 `add()` 역시 동일한 트랜잭션에 참여합니다.\
이때 내부 호출 중 예외가 발생하면 전체 작업이 롤백됩니다.

***

#### 트랜잭션 매니저와 트랜잭션 동기화

트랜잭션 추상화의 핵심은 `PlatformTransactionManager`입니다.\
트랜잭션 매니저는 트랜잭션 시작, 커밋, 롤백을 일관되게 제어합니다.

트랜잭션 동기화 기술은 현재 실행 중인 트랜잭션 정보를 보관하고, DAO 계층에서 이를 공유할 수 있게 합니다.\
이 덕분에 `JdbcTemplate`과 같은 데이터 액세스 기술도 현재 트랜잭션에 자동으로 참여합니다.

***

#### 테스트에서 트랜잭션 매니저 사용

테스트에서 여러 서비스 메서드를 하나의 트랜잭션으로 묶고 싶다면 트랜잭션 매니저를 직접 사용할 수 있습니다.
...
위 코드에서는 세 개의 서비스 호출이 하나의 트랜잭션 안에서 실행됩니다.

***

#### 트랜잭션 동기화 검증

전파 속성이 `REQUIRED`라면 이미 시작된 트랜잭션에 참여합니다.\
따라서 테스트 코드에서 먼저 트랜잭션을 시작하면 이후 호출되는 서비스 메서드는 모두 동일한 트랜잭션에 참여합니다.

이를 검증하기 위해 읽기전용 트랜잭션을 강제로 적용할 수 있습니다.

```kotlin
@Test
fun transactionSyncReadOnly() {
    val txDefinition = DefaultTransactionDefinition()
    txDefinition.isReadOnly = true

    val status = transactionManager.getTransaction(txDefinition)

    userService.deleteAll() // 쓰기 작업 → 예외 발생 예상

    transactionManager.commit(status)
}
```

읽기전용 트랜잭션에서 쓰기 작업이 발생하면 예외가 발생합니다.\
이를 통해 해당 메서드가 동일한 트랜잭션에 참여했는지 확인할 수 있습니다.

***

#### 롤백 테스트

테스트에서는 트랜잭션을 강제로 롤백하여 DB 상태를 유지할 수 있습니다.
...
롤백 이후 DB 상태가 원래대로 복구되었는지 검증합니다.

***

### 6.8.3 테스트를 위한 트랜잭션 애노테이션

스프링 테스트 프레임워크는 테스트 메서드에 `@Transactional`을 적용할 수 있습니다.
...
테스트가 종료되면 기본적으로 트랜잭션은 자동으로 롤백됩니다.\
DB를 사용하는 테스트에서 매우 유용한 기능입니다.

***

#### @Rollback

`@Transactional`이 적용된 테스트는 기본적으로 롤백됩니다.\
만약 커밋을 원한다면 `@Rollback(false)`를 사용할 수 있습니다.
