...
Trie는 검색을 뜻하는 **retrieval** 에서 용어를 따왔습니다.  Trie는 문자열을 효율적으로 저장하고 검색할 수 있는 자료구조입니다. 검색시 시간복잡도는 O(m\*26) (m은 검색어의 길이)를 가지고, 동일한 접두사를 가지는 문자열은 한번만 저장됨므로 메모리 사용량을 최적화 할 수 있습니다.
