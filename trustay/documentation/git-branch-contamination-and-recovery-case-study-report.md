...
1. 기능 개발 중 실수로 `dev` 브랜치를 `feat1(feature/PLACE-1058)` 브랜치에 잘못 가져오게 되었습니다.
2. 이후 별다른 인지 없이 `feat1` 브랜치를 `master`에 머지하였고, 이 과정에서 **의도하지 않은 dev 커밋들까지 모두 master에 포함**되었습니다.
3. 이를 발견한 후 master에서 revert를 수행해 dev 오염 커밋들을 되돌렸습니다.
4. 하지만 revert 이후 다시 `feat1` 브랜치로 PR을 생성하자, Git에서는 “변경된 내용이 없다”는 메시지가 출력되었고, 정상적인 PR 생성이 불가능한 상황이 되었습니다.
...
즉, **기능 코드만 남기고 dev 오염 히스토리를 제거한 상태로 master에 적용하는 것**이 핵심 목표였고, 이를 어떻게 해결할지 여러 시도와 실험을 거쳐야 했습니다.
...
1. **`feat1`에 `dev`를 잘못 merge/pull 하여 dev 커밋이 섞임**
   * 원래 feat1은 master 기반으로만 만들어져야 했으나,
   * 실수로 dev를 끌어오면서 완전히 다른 계열의 커밋들이 feat1 히스토리에 포함되었습니다.
2. **오염된 feat1이 master로 merge됨**
   * 이 과정에서 master에도 dev 오염 커밋들이 모두 들어갔습니다.
3. **master에서 revert 수행**
...
> “feat1에서 들어온 커밋은 master에 이미 merge된 적 있으며,\
> revert로 지워진 상태이므로\
> 다시 merge할 diff는 없다.”
...
결론적으로 가장 현실적인 가설은 **C는 반드시 작동한다**였고, 실제로도 유효한 방법이었습니다.
...
1. master 최신화
2. clean 브랜치 생성 (feature/PLACE-1058-clean)
...
위의 여러 방법 가운데, **현재 상황에서는 diff patch 방식이 절대적으로 최선의 선택**이었습니다.

선정 이유는 다음과 같습니다.

1. revert 기록 때문에 squash/normal merge가 일절 불가능
2. feat1의 dev 오염 커밋이 너무 많아 rebase 전략이 비현실적
...
1. master에서 깨끗한 신규 브랜치(feature/PLACE-1058-clean)를 생성할 수 있었습니다.
2. 오염된 커밋 히스토리는 완전히 제거하거나 독립시킬 수 있었습니다.
3. feat1-backup과 master의 실제 코드 차이만 clean 브랜치에 정확하게 반영되었습니다.
4. patch 적용 과정에서 dev에서 잘못 끌려온 코드도 자연스럽게 충돌 과정에서 제거되었습니다.
5. PR을 생성했을 때 diff가 매우 깔끔해졌으며,\
   기능 변경사항만 명확하게 표시되는 정상적인 PR이 완성되었습니다.
6. 리뷰어 입장에서 훨씬 이해하기 쉬운 코드 구조가 되었고,\
   master 기준으로도 안정적으로 merge 가능한 상태가 되었습니다.

결국, **오염된 브랜치를 더 이상 끌고 가지 않고, 기능 변경만 정확하게 선별하여 재적용하는 데 완전히 성공한 사례**였습니다.
...
3.  master를 기반으로 새로운 clean 브랜치를 생성합니다.
    ...
    ```
    git checkout -b feature/PLACE-1058-clean
    ```
...
```
git checkout feature/PLACE-1058-backup
```

master와 feat1-backup 브랜치의 **순수 파일 변화(diff)** 를 하나의 patch 파일로 저장합니다.

```
git diff master..feature/PLACE-1058-backup > PLACE-1058.patch
```
...
1.  clean 브랜치로 다시 이동합니다.
    ...
    ```
    git checkout feature/PLACE-1058-clean
    ```
2.  patch를 적용합니다.

    ```
    git apply PLACE-1058.patch
    ```
3.  충돌이 발생할 경우, 자동 적용 가능한 부분은 적용하고 나머지는 `.rej` 파일로 떨어집니다.
    ...
    ```
    git apply --reject --whitespace=fix PLACE-1058.patch
    ```
4. `.rej` 파일이 생성된 경우, 해당 파일의 내용을 참조하여\
   **dev에서 잘못 들어온 코드가 아닌 실제 기능 코드만 clean 브랜치에 수동 반영**합니다.
...
3.  파일을 하나씩 확인하며 다음 원칙으로 검증합니다.
    ...
    * PLACE-1058 기능 개발 중 직접 작성한 코드
    * 실제로 intentional하게 수정한 로직, API, 리팩터링 등

    #### ❌ 제거해야 할 내용

    * dev에서 끌려왔던 오염 코드
    * revert 이전에 master에 잘못 들어갔다가 다시 사라졌던 흔적
    * feat1 히스토리 오염 과정에서 생긴 불필요한 변경
4.  clean 브랜치에서 직접 코드를 정리하고 `git add` 합니다.
...
```
git add .
git commit -m "feat(PLACE-1058): 마스터 기준으로 기능 변경사항을 깨끗하게 재적용"
```
...
1.  clean 브랜치를 원격 저장소에 푸시합니다.
    ...
    ```
    git push -u origin feature/PLACE-1058-clean
    ```
2. GitLab/GitHub에서 PR을 생성합니다.
   * Base: master
   * Compare: feature/PLACE-1058-clean
3. 이 PR은 dev 오염 히스토리 없이 **기능 코드만 명확하게 포함한 간결한 diff** 로 표시됩니다.
